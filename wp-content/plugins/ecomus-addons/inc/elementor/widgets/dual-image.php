...
class Dual_Image extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_group_control(
			Group_Control_Image_Size::get_type(),
			[
				'name' => 'third_image', // Usage: `{name}_size` and `{name}_custom_dimension`, in this case `image_size` and `image_custom_dimension`.
				'default' => 'full',
				'condition' => [
					'third_image[url]!' => '',
				],
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', [
			...
			'em-relative' ] );
		$this->add_render_attribute( 'primary-image', 'class', [ 'ecomus-dual-image__primary-image', esc_attr( $classes ) ] );
		$this->add_render_attribute( 'secondary-image', 'class', [ 'ecomus-dual-image__secondary-image', esc_attr( $classes ) ] );
		$this->add_render_attribute( 'third-image', 'class', [ 'ecomus-dual-image__third-image', esc_attr( $classes ) ] );
		?>

		<div <?php echo $this->get_render_attribute_string( 'wrapper' ); ?>>
			<div <?php echo $this->get_render_attribute_string( 'primary-image' ); ?>>
				<?php  if ( $settings['primary_link_to'] == 'custom' && $settings['primary_link']['url'] ) : ?>
					<a href="<?php echo esc_url( $settings['primary_link']['url'] ) ?>">
				<?php endif; ?>
				<?php
					if ( $settings['primary_image']['url'] ) {
						...
						$image_size = !empty($settings['primary_image_size']) ? $settings['primary_image_size'] : 'full';
						echo \Ecomus\Addons\Helper::get_responsive_image_elementor( $image_args, $image_size );
					}
				?>
				<?php  if ( $settings['primary_link_to'] == 'custom' && $settings['primary_link']['url'] ) : ?>
					</a>
			...
			<div <?php echo $this->get_render_attribute_string( 'secondary-image' ); ?>>
				<?php  if ( $settings['secondary_link_to'] == 'custom' && $settings['secondary_link']['url'] ) : ?>
					<a href="<?php echo esc_url( $settings['secondary_link']['url'] ) ?>">
				<?php endif; ?>
					<?php
						if ( $settings['secondary_image'] ) {
							...
							$image_size = !empty($settings['secondary_image_size']) ? $settings['secondary_image_size'] : 'full';
							echo \Ecomus\Addons\Helper::get_responsive_image_elementor( $image_args, $image_size );
						}
					?>
				<?php  if ( $settings['secondary_link_to'] == 'custom' && $settings['secondary_link']['url'] ) : ?>
			...
			<?php if ( $settings['third_image']['url'] ) : ?>
				<div <?php echo $this->get_render_attribute_string( 'third-image' ); ?>>
					<?php  if ( $settings['third_link_to'] == 'custom' && $settings['third_link']['url'] ) : ?>
						<a href="<?php echo esc_url( $settings['third_link']['url'] ) ?>">
					<?php endif; ?>
						<?php
							if ( $settings['third_image'] ) {
								...
								$image_size = !empty($settings['third_image_size']) ? $settings['third_image_size'] : 'full';
								echo \Ecomus\Addons\Helper::get_responsive_image_elementor( $image_args, $image_size );
							}
						?>
					<?php  if ( $settings['third_link_to'] == 'custom' && $settings['third_link']['url'] ) : ?>
