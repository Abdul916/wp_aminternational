(function ($) {
	...
	ecomus.productQuickAdd = function() {
		...
		ecomus.$body.on( 'click', '.product_type_variable[data-target="quick-add-modal"]', function( event ) {
			event.preventDefault();
	...
	ecomus.recentlyViewedProducts = function () {
		...
		if ( ! $recently.hasClass( 'has-ajax' ) ) {
			getProductCarousel( $recently, $recently_columns );
			return;
		}
					...
					if( response.success ) {
						...
						if ( $recently.find( '.no-products' ).length < 1 ) {
							if( ! $recently.data( 'settings' ) ) {
								getProductCarousel( $recently, $columns );
							} else {
		...
		function getProductCarousel( $productSection, $columns ) {
